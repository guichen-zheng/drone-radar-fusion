"""
yolo_worker.py — 在 yolov8 conda 环境里跑推理（Python 3.8 + ultralytics 8.4 + cv2 4.13）
...
def main():
    ...
    out_stream = _ipc_out  # 用前面 dup 出来的纯净 fd，避免 stdout 被污染
    ...
    while True:
        ...
        if frame_idx <= 3 or frame_idx % 30 == 0:
            sys.stderr.write(f"[yolo_worker] 收到帧 #{frame_idx} ({n} 字节)\n"); sys.stderr.flush()
